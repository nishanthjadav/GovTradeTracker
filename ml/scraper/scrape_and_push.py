"""
Scrapes CapitolTrades and pushes new trades to the backend ingest endpoint.
...
def scrape(stop_on_duplicate_page: bool = True) -> list[dict]:
    ...
    while True:
        ...
        try:
            ...
            if stop_on_duplicate_page and not page_trades:
...
def push(trades: list[dict], backend_url: str, secret: str) -> None:
    if not trades:
        log.info("No trades to push.")
        return

    # Push in batches so we don't send one giant request
    batch_size = 50
    ...
    for i in range(0, len(trades), batch_size):
        ...
        log.info("Batch %d/%d pushed — saved: %d, skipped: %d, errors: %d",
                 i // batch_size + 1, -(-len(trades) // batch_size),
                 data.get("saved"), data.get("skipped"), data.get("errors"))

    log.info("Push complete — total saved: %d, skipped: %d, errors: %d",
...
def main() -> int:
    ...
    trades = scrape(stop_on_duplicate_page=True)
    log.info("Scraped %d trades total.", len(trades))
    push(trades, backend_url, secret)
    return 0
