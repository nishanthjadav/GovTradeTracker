...
export default function LeaderboardPage({
  ...
  const ranked = useMemo(() => {
    ...
    for (const t of trades) {
      ...
      if (!s) {
        s = { count: 0, buys: 0, sells: 0, totalSize: 0 };
      ...
      if (size > 0 && size < 1e15) s.totalSize += size;
    }
    ...
    const rows = politicians
      ...
      .map((p) => {
        const s = stats.get(p.id) || { count: 0, buys: 0, sells: 0, totalSize: 0 };
    ...
    <>
      ...
      <div className="filter-bar">
        ...
        <div className="filter-group">
          ...
          <div className="filter-pills">
            <button
              className={`filter-pill ${partyFilter === "all" ? "active" : ""}`}
              onClick={() => setPartyFilter("all")}
            >
              All
            </button>
            <button
              className={`filter-pill dem ${partyFilter === "democrat" ? "active" : ""}`}
              onClick={() => setPartyFilter("democrat")}
            >
              Dem
            </button>
            <button
              className={`filter-pill rep ${partyFilter === "republican" ? "active" : ""}`}
        ...
        <div className="filter-group">
          ...
          <div className="filter-pills">
            <button
              className={`filter-pill ${chamberFilter === "all" ? "active" : ""}`}
              onClick={() => setChamberFilter("all")}
            >
              All
            </button>
            <button
              className={`filter-pill ${chamberFilter === "house" ? "active" : ""}`}
              onClick={() => setChamberFilter("house")}
            >
              House
            </button>
            <button
              className={`filter-pill ${chamberFilter === "senate" ? "active" : ""}`}
      ...
      <div className="trades-table-scroll">
        <div className="trades-table" style={{ minWidth: 480 }}>
          <div
            className="table-header"
            style={{ gridTemplateColumns: "48px minmax(180px,1fr) 140px" }}
          >
            <div>#</div>
            <div>Politician</div>
